...
export const signup = async(req, res) => {
    if (!req.body.email || !req.body.password) { // Si falta algún campo en el registro...
        return res.status(400).send({ message: 'email y contraseña necesarios' })
    }
    try {
    ...
    } catch (error) { // En caso de algún error...
        console.error(error)
        if (error.code === 11000) {
            return res.status(400).send({ error: 'El correo ya se encuentra registrado' })
        }
...
export const signin = async(req, res) => {
    if (!req.body.email || !req.body.password) { // Si falta algún campo en la solicitud...
        return res.status(400).send({ message: 'email y contraseña necesarios' })
    }
    ...
    try { // Si encontramos al usuario...
        const match = await user.checkPassword(req.body.password) // Verificamos que la contraseña coincida
        if (!match) {
            return res.status(401).send({ message: 'La contraseña no coincide' }) // La contraseña no coincide
        }
        ...
        console.error(error)
        return res.status(401).send({ message: 'Ocurrió un error, vuelve a intentarlo de nuevo' })
    }
}
...
export const protect = async(req, res, next) => {
    ...
    if (!req.headers.authorization) { // Si la petición no contiene el header de authorization...
    ...
    if (!token) { // Si el header no contiene el token...
    ...
    try {
        ...
        console.error(error)
        return res.status(401).send({ message: 'Ocurrió un error, verifica que tengas acceso' })
    }
}